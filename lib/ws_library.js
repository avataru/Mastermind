...
var applyWeights = function (db, players) {
    // Cleanup
    const playerIds = _.map(players, 'user.id').join(',');
    db.query(`DELETE FROM white_star_weights WHERE userId NOT IN (${playerIds})`,
        function (error) { if (error) { return console.log(`Unable to cleanup the old players`, error.message); }}
    );
    ...
    _.forEach(players, function (player) {
        ...
        if (_.some(dbPlayers, ['userId', player.user.id])) {
            weight = _.find(dbPlayers, ['userId', player.user.id]).weight;
        } else {
            db.query(`INSERT IGNORE INTO white_star_weights (userId, username) VALUES ('${userId}', '${username}')`,
                function (error) { if (error) { return console.log(`Unable to add player's weight`, error.message); }}
            );
        }
    ...
    return _.uniqBy(_.shuffle(entries), 'user.id');
}

var clearDraw = function (db) {
    db.query(`DELETE FROM white_star`, [], function(error) {
        if (error) {
...
var addPlayerRow = function(table, row, team) {
    ...
    let confirmedText = team !== UNDRAWN_TEAM_NAME
        ? (row.confirmed === CONFIRM_YES ? '[Y]' : '[N]')
        : '[-]';
