...
var addNomination = function(db, nominatorUserId, userId, username, achievementId, timestamp) {
    db.query(`INSERT OR IGNORE INTO nominations (nominatorUserId, userId, username, achievementId, timestamp)
...
var addAchievement = function(db, userId, username, achievementId) {
    db.beginTransaction((error) => {
        ...
        db.query(`INSERT OR IGNORE INTO achievements (userId, username, achievementId, num) VALUES ('${userId}', '${username}', '${achievementId}', 0);`, function(error) {
            if (error) {
                return console.log(`Unable to add achievement for user ${userId}`, error.message);
            }
...
var getAchievements = function(db, userId, callBack) {    
    db.query(`SELECT username, achievementId, SUM(num) AS num, SUM(nom) AS nom FROM (
