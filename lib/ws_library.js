...
var addDrawnPlayer = function(db, userId, username, team, confirmed) {
    db.query(`INSERT OR IGNORE INTO white_star (userId, username, team, confirmed)
...
var updateDrawnPlayer = function(db, userId, team, confirmed) {
    db.query(`UPDATE OR IGNORE white_star SET team = '${team}', confirmed = '${confirmed}' WHERE userId = '${userId}'`,
            function(error) {
                if (error) {
...
var confirmPlayerDraw = function(db, userId, confirmed) {
    db.query(`UPDATE OR IGNORE white_star SET confirmed = '${confirmed}' WHERE userId = '${userId}'`,
            function(error) {
                if (error) {
                    return console.log(`Unable to update drawn user ${userId}`, error.message);
                }
...
var getDrawnPlayers = function(db, callBack) {    
    db.query(`SELECT userId, username, team, confirmed FROM white_star ORDER BY team, username;`, [], (error, rows) => {
        if (error) {
